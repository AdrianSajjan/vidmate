  ...
  return (
    <div className={cn("flex flex-1 shrink select-none", editor.timelineOpen ? "h-auto" : "h-0 overflow-hidden appearance-none")}>
      ...
      <div className="flex-1 flex flex-col bg-background shrink-0 overflow-x-scroll scrollbar-hidden relative" ref={containerRef}>
        ...
        <Draggable axis={disabled ? "none" : "x"} position={{ y: 0, x: (editor.canvas.timeline.seek / 1000) * SEEK_TIME_WIDTH }} bounds={{ left: 0, right: trackWidth }} onStop={(_, data) => onSeekHandleDrag(data.x)}>
          <div className={cn("absolute h-full w-1 bg-blue-400 dark:bg-primary z-20", disabled ? "cursor-not-allowed" : "cursor-ew-resize")} />
        </Draggable>
...
function _TimelineElementItem({ element, trackWidth }: { element: fabric.Object; trackWidth: number }) {
  ...
  const disabled = editor.canvas.timeline.playing || editor.canvas.animations.previewing;
  const style = { width, backgroundImage: `url(${backgroundURL})`, backgroundSize: `${backgroundWidth}px 40px` };
  ...
  return (
    <div className="h-10 overflow-visible shrink-0 relative">
      {isSelected ? (
        <Draggable axis={disabled ? "none" : "x"} bounds={{ left: 0, right: offset + width - HANDLE_WIDTH * 2 }} position={{ y: 0, x: offset }} onDrag={(_, data) => handleDragLeftBar(data.x)}>
      ...
      <Draggable
        ...
        onMouseDown={(event) => (disabled ? null : editor.canvas.selection.selectObjectByName(element.name!, event.shiftKey))}
      >
      ...
      {isSelected ? (
        <Draggable axis={disabled ? "none" : "x"} bounds={{ left: offset + HANDLE_WIDTH, right: trackWidth }} position={{ y: 0, x: offset + width }} onDrag={(_, data) => handleDragRightBar(data.x)}>
...
function _TimelineAudioItem({ audio, trackWidth }: { audio: EditorAudioElement; trackWidth: number }) {
  ...
  const disabled = editor.canvas.timeline.playing || editor.canvas.animations.previewing;
  const style = { width, backgroundImage: `url(${backgroundURL})`, backgroundSize: `${width}px 40px` };
  ...
  return (
    <div className="h-10 overflow-visible shrink-0 relative">
      {isSelected ? (
        <Draggable axis={disabled ? "none" : "x"} bounds={{ left: Math.max(0, width + offset - duration), right: offset + width - HANDLE_WIDTH * 2 }} position={{ y: 0, x: offset }} onDrag={(_, data) => handleDragLeftBar(data.x)}>
          <button className="flex items-center justify-center bg-primary absolute top-0 h-full z-10 rounded-l-lg cursor-ew-resize" style={{ width: HANDLE_WIDTH }}>
      ...
      <Draggable
        ...
        position={{ y: 0, x: offset }}
        onDrag={(_, data) => handleDragTrack(data.x)}
        onMouseDown={() => (disabled ? null : editor.canvas.selection.selectAudio(isSelected ? null : audio))}
      >
        <button className={cn("absolute top-0 h-full z-0 border-3 rounded-lg overflow-hidden cursor-grab active:cursor-grabbing", isSelected ? "border-primary" : "border-gray-400")} style={style}>
          <span className={cn("absolute top-1 bg-foreground/50 text-card rounded-sm backdrop-blur-sm px-2 py-1 flex items-center gap-2.5 capitalize", isSelected ? "left-5" : "left-1")}>
      ...
      {isSelected ? (
        <Draggable axis={disabled ? "none" : "x"} bounds={{ left: offset + HANDLE_WIDTH, right: Math.min(offset + duration, trackWidth) }} position={{ y: 0, x: offset + width }} onDrag={(_, data) => handleDragRightBar(data.x)}>
          <button className="inline-flex items-center justify-center bg-primary absolute top-0 h-full z-10 rounded-r-lg cursor-ew-resize" style={{ width: HANDLE_WIDTH, left: -HANDLE_WIDTH }}>
